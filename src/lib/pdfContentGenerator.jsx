// src/lib/pdfContentGenerator.jsx

/**
 * In diesem Modul befinden sich alle Funktionen, die
 * den HTML-Code für Seite 1 und Seite 2 generieren.
 * Wichtig: getPage1Content greift u. a. auf data.cartItems, data.totalWeight und data.totalPrice zu.
 */
...
const generateItemsTable = (cartItems, totalWeight, totalPrice) => {
  // Erwartet, dass cartItems ein Array ist; verwendet map().
...
const generatePaymentInfoPage1 = (data) => {
  ...
  if (deliveryType === 'selbstanlieferung') {
    if (!iban && !paypal)
      paymentInfoHtml += '<p>Auszahlung erfolgt vor Ort in Bar oder nach Vereinbarung.</p>';
  } else {
    if (!iban && !paypal)
      paymentInfoHtml += '<p>Keine Bank-/PayPal-Daten für Auszahlung angegeben.</p>';
  }
...
const generateSpecificDeliveryDetailsPage1 = (data) => {
  ...
  if (deliveryType === 'abholung' && pickupDetails) {
    if (pickupDetails.date) {
      specificDetailsHtml += `<p><strong>Abholung am:</strong> ${new Date(
        pickupDetails.date
        ...
        month: 'short',
        day: 'numeric',
      })}${pickupDetails.time ? ` / ${pickupDetails.time} Uhr` : ''}</p>`;
    } else {
      specificDetailsHtml += `<p><strong>Abholung:</strong> Wir kontaktieren Sie zur Terminvereinbarung.</p>`;
    }
    if (pickupDetails.notes)
      specificDetailsHtml += `<p><strong>Anmerkungen:</strong> ${pickupDetails.notes}</p>`;
  } else if (deliveryType === 'selbstanlieferung') {
    if (deliveryDate) {
      specificDetailsHtml += `<p><strong>Anlieferung am:</strong> ${new Date(
        deliveryDate
      ).toLocaleDateString('de-DE', {
        weekday: 'short',
        year: 'numeric',
    ...
    if (selfDeliveryNotes)
      specificDetailsHtml += `<p><strong>Anmerkungen:</strong> ${selfDeliveryNotes}</p>`;
...
const generatePage1Header = () => `
  <div class="header">
    <h1>
      <svg class="logo-inline" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <!-- (SVG-Icon-Inhalt der Buchretter-Logo) -->
      </svg>
...
const generateQrAndSignatureSectionPage1 = (qrCodeDataURL, ankaufsNummer) => `
  <div class="qr-signature-section">
    <div class="qr-code-container">
      ...
      ${
        qrCodeDataURL
          ? `<img src="${qrCodeDataURL}" alt="QR Code für Ankaufsnummer ${ankaufsNummer}"/>`
...
/**
 * Gibt den HTML-Code für Seite 1 des PDFs zurück.
 * Erwartet, dass data.cartItems, data.totalWeight, data.totalPrice usw. definiert sind.
 */
  ...
  if (deliveryType === 'versand') {
    content += `
      <div>
        ...
        <ol>
          ...
          <li><strong>Versandetikett (ab 10 kg):</strong> Wenn deine Sendung 10 kg oder mehr wiegt und du ein kostenloses Label von uns erhalten hast (per E-Mail oder Download), klebe dieses gut sichtbar auf das Paket.</li>
          <li><strong>Versand (unter 10 kg oder ohne unser Label):</strong> Frankiere das Paket bitte ausreichend und sende es an: Die Buchretter GbR, Triftstr. 21B, 16348 Wandlitz OT Klosterfelde.</li>
    ...
    content += `
      <div>
        ...
        <ol>
          <li><strong>Dein Termin:</strong> ${deliveryDate
...
/**
 * Gibt den HTML-Code für Seite 2 des PDFs zurück.
 */

...
import { usePdfUpload } from '@/hooks/usePdfUpload.jsx'; // Hook mit dem neuen PDF-Export
...
const ConfirmationPage = () => {
  ...
  // Beim Mount: Bestätigungsdaten aus location.state oder localStorage holen
  useEffect(() => {
        ...
        try {
          ...
          if (canvas) {
            ...
            if (
              ...
              dataUrl.length > 'data:image/png;base64,'.length + 50
            ) {
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        'ConfirmationPage: Triggering PDF generation and upload. QR is ready:',
        isQrReadyForPdf,
        'Ankaufsnummer:',
        ankaufsNummer,
        'QR Data URL length:',
        qrCodeDataURLForEmailAndPdf.length
      );
    ...
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <p className="text-muted-foreground text-lg mb-8">
          Es konnten keine Bestätigungsdaten geladen werden. Bitte versuche den Ankauf erneut.
        </p>
        <Button asChild size="lg" className="bg-primary hover:bg-primary/90 text-lg py-3 px-6">
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      >
        <Card className="max-w-3xl mx-auto shadow-2xl overflow-hidden border-2 border-emerald-500">
          <CardHeader className="bg-gradient-to-br from-emerald-500 to-green-600 text-white p-8">
            ...
            <CardTitle className="text-4xl md:text-5xl font-bold text-center tracking-tight">
              Ankauf Erfolgreich!
            </CardTitle>
          ...
          <CardContent className="p-6 md:p-8 space-y-8">
            ...
            <BegleitscheinSection
              submissionData={{ ...confirmationData, ankaufsNummer }}
              autoOpenPdf={true}
            ...
            <div className="text-center pt-6 mt-6 border-t-2 border-dashed border-gray-200 space-y-4">
              <Button
                asChild
                size="lg"
                className="bg-primary hover:bg-primary/90 text-lg py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-300"
              >
                <Link to="/">
      ...
      {ankaufsNummer && (
        <div style={{ display: 'none' }}>
          {/* Unsichtbarer QR-Code-Canvas für PDF-Generierung */}
          <QRCodeCanvas
            id="qrCodeCanvasHiddenConfirmation"
            value={ankaufsNummer}
            size={256}
            level="H"
